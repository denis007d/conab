...
# FUNÇÃO CORRIGIDA: Determinar tipo de cargo 
def determinar_tipo_cargo(nome_cargo: str, codigo_cargo: str = "", cargos_dict: dict = None) -> str:
    """
    Determina o tipo de cargo baseado no nome e código do cargo.
    Prioriza o código se disponível, senão usa o nome.
    """
    # Se temos acesso ao dicionário de cargos e código, usar isso primeiro
    if cargos_dict and codigo_cargo and codigo_cargo in cargos_dict:
        cargo_info = cargos_dict[codigo_cargo]
        nome_cargo = cargo_info.get('nome', nome_cargo)
        # Verificar se há um tipo específico definido no JSON
        if 'tipo' in cargo_info:
            return cargo_info['tipo']
    ...
    # Fallback: assumir analista comum
    return "ANALISTA_COMUM"

# FUNÇÃO CORRIGIDA: Obter configuração de matérias baseada no cargo
def obter_config_materias(cargo_nome: str, codigo_cargo: str = "", cargos_dict: dict = None) -> dict:
    """
    Retorna a configuração de matérias para um cargo específico.
    """
    tipo_cargo = determinar_tipo_cargo(cargo_nome, codigo_cargo, cargos_dict)
    
    if tipo_cargo not in MATERIAS_CONFIG:
        # Se não encontrar, usar configuração padrão
        st.warning(f"Configuração não encontrada para o tipo '{tipo_cargo}'. Usando ANALISTA_COMUM como padrão.")
        tipo_cargo = "ANALISTA_COMUM"
    
    return MATERIAS_CONFIG[tipo_cargo]

# Função para calcular nota total SEM pesos 
def calcular_nota_total(notas, cargo_nome, codigo_cargo="", cargos_dict=None): 
    return sum(notas.values())
 
# Função para verificar aprovação (mantém a lógica original dos pesos para verificar mínimos) 
def verificar_aprovacao(notas, cargo_nome, codigo_cargo="", cargos_dict=None): 
    config = obter_config_materias(cargo_nome, codigo_cargo, cargos_dict)
...
def editar_candidato(index): 
    ...
    codigo_cargo = candidato.get('cargo_codigo', '')
    config = obter_config_materias(nome_cargo, codigo_cargo, cargos)
    tipo_cargo = determinar_tipo_cargo(nome_cargo, codigo_cargo, cargos)
        ...
        if salvar: 
            if not novo_nome: 
                ...
                if novo_nome.lower() in nomes_existentes: 
                    ...
                    total_pontos = calcular_nota_total(novas_notas, nome_cargo, codigo_cargo, cargos) 
                    aprovado = verificar_aprovacao(novas_notas, nome_cargo, codigo_cargo, cargos) 
...
if pagina == "Cadastro": 
    ...
    if not cargos: 
        ...
        if cargo_selecionado:
            codigo_cargo = cargo_selecionado.split(" - ")[0] 
            nome_cargo = cargos[codigo_cargo]['nome'] if codigo_cargo in cargos else "" 
            
            # CORREÇÃO: Usar as funções corrigidas
            config = obter_config_materias(nome_cargo, codigo_cargo, cargos)
            tipo_cargo = determinar_tipo_cargo(nome_cargo, codigo_cargo, cargos)
            ...
            with st.form("form_candidato"): 
                ...
                if submitted: 
                    if not nome_candidato: 
                        ...
                        total_pontos = calcular_nota_total(notas, nome_cargo, codigo_cargo, cargos) 
                        aprovado = verificar_aprovacao(notas, nome_cargo, codigo_cargo, cargos) 
    ...
    if st.session_state.candidatos: 
        ...
        if not df_filtrado.empty: 
            ...
            # CORREÇÃO: Determinar quais matérias exibir com base no filtro de cargo 
            ...
            if cargo_filtro != "Todos": 
                ...
                config_filtro = obter_config_materias(nome_cargo_filtro, codigo_filtro, cargos)
                materias_para_exibir = config_filtro["materias"]
            else: 
                # Se não há filtro de cargo ("Todos"), exibir matérias de todos os cargos presentes no df_filtrado 
                tipos_cargo_unicos = []
                for _, row in df_filtrado.iterrows():
                    cargo_nome = row.get('cargo_nome', '')
                    codigo_cargo = row.get('cargo_codigo', '')
                    tipo_cargo = determinar_tipo_cargo(cargo_nome, codigo_cargo, cargos)
                    if tipo_cargo not in tipos_cargo_unicos:
                        tipos_cargo_unicos.append(tipo_cargo)
